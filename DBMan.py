...
def insertHeading(local_addr,net_addr):
    session = DBSession()
    heading = Heading(local_addr=local_addr, net_addr=net_addr)
    session.add(heading)
    session.commit()
    session.close()
...
def insertLung(local_addr,net_addr):
    session = DBSession()
    lung = Lung(local_addr=local_addr, net_addr=net_addr)
    session.add(lung)
    session.commit()
    session.close()
...
def insertPupil(local_addr,net_addr):
    session = DBSession()
    pupil = Pupil(local_addr=local_addr, net_addr=net_addr)
    session.add(pupil)
    session.commit()
    session.close()
